...
- **Conversor de Cenas para Vídeo MP4 com VA-API (`rice-scene-to-video`):** Utilitário CLI nativo (`~/.local/bin/rice-scene-to-video`) que converte wallpapers complexos do Wallpaper Engine (`scene.pkg`) em papéis de parede de Vídeo MP4 nativos em 1080p 60 FPS com decodificação por hardware Intel VA-API (0% CPU, 0% carga de GPU 3D). Extrai a trilha sonora original (`.wav`/`.mp3`), isola a tela via IPC do Quickshell (`setBarVisible false`, `frame.setVisible false`) e workspace vazia, grava em tempo real via `wf-recorder` e multiplexa com áudio estéreo em loop perfeito com `ffmpeg`. Atualiza automaticamente o `project.json` (mantendo `.original`) e o banco de dados do Waywallen (`waywallen-v2.db`) para reprodução imediata com o `waywallen-video-renderer`. Inclui comandos `--list` (catálogo e status de hardware de todos os wallpapers) e `--all` (conversão em lote).
