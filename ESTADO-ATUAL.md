...
|---|---|---|
| **Barra Superior** | `TopBar.qml` | Substitui a antiga Waybar. Workspaces animados em formato pill, título da janela ativa, relógio central que abre o Hub ao clicar, e popups com cantos invertidos para Áudio, Wi-Fi, Bluetooth e Bateria. Em tela cheia regular (jogos/vídeos), fica oculta sob a janela. Ao abrir o Launcher (`Super` ou `Super + R`), eleva-se dinamicamente para a camada `WlrLayer.Overlay`, ficando visível sobre o jogo/vídeo. |
...
| **Launcher de Apps** | `Launcher.qml` | Abre com `Super + R` ou tecla `Super` isolada. Lista vertical alfabética (A → Z) de cima para baixo com scrollbar sutil de 4px, sensibilidade de rolagem rápida, pesquisa instantânea, menu de contexto com botão direito (fixar na dock, adicionar aos jogos). Máscara de entrada configurada para liberar a TopBar, o gatilho da Sidebar e da Dock. |
| **Dock Inferior** | `Dock.qml`, `DockConfig.qml` | Retrátil com auto-hide (revela ao encostar o mouse na borda inferior). Em tela cheia regular, permanece inativa no `WlrLayer.Top`. Ao abrir o Launcher, eleva-se dinamicamente para `WlrLayer.Overlay`, permitindo visualização e acesso mesmo durante jogos ou vídeos em tela cheia. |
| **Barra de Energia** | `EnergySidebar.qml`, `rice-session-action` | Barra retrátil na borda lateral direita. Em tela cheia regular, o hover é 100% desativado e a máscara de cliques é zerada (evita abertura acidental durante jogos). Ao abrir o Launcher (`Super`/`Super+R`), o hover é reativado sobre a tela cheia. Contém avatar animado, contagem de atualizações de pacotes, atalhos de luz noturna e status da dGPU NVIDIA, system trays integrados e controles de energia. |
