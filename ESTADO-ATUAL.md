...
| **Painel de Configurações Visuais (Rice Control Center)** | `VisualConfigPanel.qml`, scripts `rice-*-apply`, `rice-maintenance`, `rice-wallust-refresh` | Central gráfica completa com barra de navegação lateral (230px) e área de conteúdo espaçosa, aberta pelo ícone de sintonia (`Theme.icons.tune`) na TopBar, pelo atalho universal **`Super + I`** ou pelo menu de apps. Totalmente projetada para usuários iniciantes e avançados configurarem recursos sem precisar tocar no terminal: **1. Fastfetch** (galeria visual de logos e GIFs em `~/Imagens/FastFetch` com detecção de ativo, sliders de largura/altura, botões interativos para ativar/ocultar módulos e visualizador no Kitty); **2. Kitty Terminal** (opacidade, tamanho da fonte, window padding, seletor de cursor beam/block/underline, blur e áudio bell); **3. Mako Notificações** (seletor matricial 3x2 de posição na tela, timeout, arredondamento, espessura da borda e botão de teste imediato); **4. Tela & Monitores** (detecta display `eDP-1`, alternância 1-clique entre **144 Hz (Ultra Suave)** e **60 Hz (Economia)**, toggle de **VRR / FreeSync Adaptativo** e slider suave de brilho da tela); **5. Teclado & Mouse** (seletor visual entre **ABNT2 (Brasil)** e **US Internacional**, slider de sensibilidade do mouse de -1.0 a +1.0, perfil de aceleração **Flat (1:1 sem aceleração)** vs **Adaptativo**, e NumLock automático no boot); **6. Cores & Wallust** (visualização das cores principais e da grade de 16 cores ANSI da paleta do wallpaper com clique para copiar código HEX para o clipboard, botão para regenerar cores do papel de parede ativo e atalho para o carrossel Waywallen); **7. Efeitos & Hyprland** (luz noturna com temperatura em Kelvin, escurecimento de janelas inativas, rounding, gaps, presets de animação e central integrada de SDDM & Limine); **8. Sistema & Reparo** (auto-diagnóstico e reparo em 1 clique: reiniciar áudio PipeWire, destravar pacman removendo `db.lck`, limpeza segura de caches de pacotes antigos e inicializador do Rice Doctor). |
