    ...
    Component {
        ...
        IconButton {
            ...
            Component.onCompleted: {
                var idx = (typeof sessionModel !== "undefined" && sessionModel.lastIndex >= 0) ? sessionModel.lastIndex : 0;
                var sName = "";
                if (typeof sessionModel !== "undefined" && sessionModel.rowCount() > 0) {
                    sName = sessionModel.data(sessionModel.index(idx, 0), 260) || "";
                }
                if (!sName) sName = "Hyprland";
                if (showLabel) {
                    label = sName;
                }
                var lower = sName.toLowerCase();
                var sIcon = "../icons/sessions/default.svg";
                if (lower.includes("plasma")) sIcon = "../icons/sessions/kde.svg";
                else {
                    var available = ["hyprland", "kde", "gnome", "ubuntu", "sway", "awesome", "qtile", "i3", "bspwm", "dwm", "xfce", "cinnamon", "niri"];
                    for (var i = 0; i < available.length; i++) {
                        if (lower.includes(available[i])) {
                            sIcon = "../icons/sessions/" + available[i] + ".svg";
                            break;
                        }
                    }
                }
