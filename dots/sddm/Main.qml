...
Item {
    ...
    Item {
        ...
        // Zoom lento no fundo ao sair da tela de bloqueio para a de login: o
        // fundo "se aproxima" enquanto o desfoque aumenta, dando a sensação de
        // profundidade que faltava (antes só trocava de camada, seco).
        property real bgZoom: root.state === "loginState" ? 1.05 : 1.0
        Behavior on bgZoom {
            enabled: Config.enableAnimations
            NumberAnimation { duration: 700; easing.type: Easing.OutCubic }
        }

        // Entrada suave do greeter inteiro (evita o "estala na tela" no boot).
        opacity: 0
        NumberAnimation on opacity {
            running: true
            from: 0
            to: 1
            duration: Config.enableAnimations ? 500 : 0
