...
- **Media tab with two layouts**: *Disc* — a vinyl that spins while music plays, scrolling title, output-device and player chips, and the full equalizer underneath; or *Ring* — cover art wrapped in a live spectrum ring, controls in the middle, and a side panel that switches between lyrics and the equalizer. One button in the tab flips between them and the choice is remembered.
- **10-band equalizer (`rice-eq`)**: built on PipeWire's own `filter-chain` biquads — no EasyEffects, nothing extra to install. It runs as a transient user unit and registers as a WirePlumber *smart filter*, so it slots in front of whatever output is the default (speakers, Bluetooth headset, HDMI) and follows it when that changes, while the volume controls keep seeing the real device. Gains change live without cutting the audio; eight presets plus custom bands (drag, scroll, double-click to reset), with automatic pre-gain so boosts don't clip.
...
- **Wallpaper transitions (`rice-wallpaper-fade`)**: switching an animated wallpaper kills one renderer and starts another, which costs anywhere from a few frames to almost a second. Instead of trying to blend two videos — which no backend supports — the screen is covered by a still of the incoming wallpaper while the outgoing one keeps animating underneath; the swap happens hidden behind it, and the layer then clears onto the new wallpaper already running. Four styles, picked in the Colours tab: **fade**, **wipe** (30° diagonal), **wave** (wipe with a rippled edge) and **grow** (circle from the centre). The masks are GPU gradients, so the repo needs no shader build step.
