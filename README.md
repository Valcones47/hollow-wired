...
- **Windows style / Hyprland style** (`rice-window-mode`, Settings → How to Use, also on the welcome screen): switch live between tiling and floating, centered windows with resize-on-border and working maximize buttons. Shortcuts stay the same. With the optional `hyprland-plugin-hyprbars` package, windows without their own title bar (terminals) get one with minimize, maximize and close.
