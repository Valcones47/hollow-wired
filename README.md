...
- **Interface layouts (`ShellLayout.qml`)**: the shell rearranges itself. Four ready-made layouts — *Classic* (top bar + hover dock), *Windows style* (full-width pinned dock and a pinned action center), *Side bar* (narrow vertical bar, the way most Hyprland rices are laid out) and *Clean screen* (everything on hover) — then every piece is adjustable on its own: where the bar sits (top, left, right), what shows up in it, whether the dock spans the full width, and which pieces reserve space for windows. Pick it from the welcome guide, from Settings → Customization, or by right-clicking the desktop.
- **Vertical bar (`VerticalBar.qml`)**: the side-bar alternative to the top bar, with workspaces, a stacked clock and the system indicators in 52px.
...
- **Rice Doctor (`rice-doctor`)**: One-click fixer (sidebar, Settings, desktop right-click menu or the launcher: "Fix problems"). Runs ~36 checks (shell services, audio, network, Bluetooth, GPU drivers incl. the Flatpak NVIDIA runtime, pacman lock, pending reboot, missing packages, fonts, disk space). Certain problems are fixed right away (asking for the password once if needed); things that might be a choice — cleaning caches, orphans, a big trash — are offered one by one, default "no". `--auto` fixes without asking, `--check`/`--json` only report.
...
- **Native Clipboard (`Clipboard.qml`)**: Built-in clipboard manager invoked with `Super + V`, supporting quick search, image preview, and instant history clearing. Picking an entry **pastes it straight into the focused field**, Windows-style (`rice-paste`, toggle in the header, `Ctrl + click` copies without pasting). Copied content survives closing the source application (`wl-clip-persist -e` plus `rice-clipboard-guard`, which hands the last history entry back when the selection is orphaned), and a dedicated **Favourites** tab keeps pinned entries out of the rolling history.
...
- **Central Hub & Control Center (`VisualConfigPanel.qml` / `Super + I`)**: **19 configuration categories** grouped into Appearance, Hardware, System and Help, with a search box that finds individual settings rather than just categories. Highlights:
  ...
  - Keyboard (10 layouts, repeat rate, touchpad), idle/power timings, gaming & GPU, storage, printers, and a first-run **Welcome guide** aimed at people arriving from Windows — six pages with side navigation, its own darker theme and the Wired eye drawn on a canvas (it blinks, looks around and glitches), where you also pick the interface layout and how windows should open.
  - A **Customization** group gathers the interface layout, the keyboard shortcuts editor (create a shortcut by pressing the keys; Discord global mute/deafen and rice actions such as GameMode, Steam, Heroic and drop-down terminal live here too) and the shell look.
...
- **Edit mode (KDE-style)**: right-click the bar (or pick "Edit mode" on the desktop menu) to drag the bar to another edge, pick what the bar and the right-hand action center show from a catalog (media, app tray, updates, night light, keep awake, record, screenshot, clipboard, NVIDIA GPU, lock, settings, power…; the top bar and the side bar keep separate lists), drag bar items to reorder, rearrange widgets and dock icons, and switch between ready-made layouts from a small, draggable floating tab. Esc or "Done" leaves it.
- **Performance profiles (`rice-perf-profile`)**: Light, Medium, Heavy or Custom in *Effects & windows*. One level sets blur, window shadows, window and workspace animations, shell panel animations and the Super + Tab live preview; Custom lets you pick each one.
- **Quick settings (Windows 11 / macOS style)**: brightness, volume and battery sit in one block on the bar; hover or click it to open a panel with big toggles (Wi‑Fi, Bluetooth, Do not disturb, game mode, blur, night light), brightness/volume/mic sliders, audio output and per-app volume, media controls, power profile, power buttons and a shortcut to Settings.
